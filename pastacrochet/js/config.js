...
PC.config = {
    ...
    // 看分數時：達到此門檻 → 換更熱鬧的高分結算 BGM（bgm_result_high）。
    // 門檻是「每盤」的：雙廚一次做兩盤、分數相加 → 實際門檻自動 ×2（220）；對戰比的是單盤最高分 → 仍是 110。
    RESULT_HIGH_SCORE: 110,
    ...
    // 鍵盤盤按錯鍵：每按錯一次扣「該點基礎分」的這個比例（點還留著，可以再按對）。針目得分最低扣到 0。
    WRONG_KEY_MULT: 0.2,
